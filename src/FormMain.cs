...
namespace VisualTranslate
{
    public partial class FormMain : Form
    {
        ...
        private void UpdateDGV()
        {
            ...
            foreach (var key in targetLanguage.Keys)
            {
            ...
            for (int i = 0; i < keyList.Count; i++)
            {
                ...
                if (!key.Contains("__comment"))
                {
                    dataGridView1.Rows.Add(key,
                        originalLanguage.ContainsKey(key) ? originalLanguage[key] : null,
                        (targetLanguage.ContainsKey(key) && targetLanguage[key] != originalLanguage[key]) ? targetLanguage[key] : null);
                }
                else
                {
                    WriteLog("已忽略指定翻译键：" + key);
                }
            }
        ...
        {
            bool originalEmpty = string.IsNullOrWhiteSpace((string)dataGridView1.Rows[rowIndex].Cells["ColumnOriginalLanguage"].Value);
            ...
            if (targetEmpty && !originalEmpty) //未翻译
            {
                dataGridView1.Rows[rowIndex].Cells["ColumnKey"].Style.ForeColor = Settings.untranslatedColor;
            }
            if (!targetEmpty && originalEmpty) //源语言不存在
            {
                dataGridView1.Rows[rowIndex].Cells["ColumnKey"].Style.ForeColor = Settings.originalNotExistColor;
            }
            if (targetEmpty && originalEmpty) //空翻译
            {
                dataGridView1.Rows[rowIndex].Cells["ColumnKey"].Style.ForeColor = Settings.emptyKeyColor;
            }
        }
        ...
        private void button_save_Click(object sender, EventArgs e)
        {
            ...
            export.Add("__comment_vt", "Generated by VisualTranslate: https://gitlab.com/MineCommander/visual-translate");
